...
class fluoSum:
    ...
    def selectivity_session(self, ifPSTH):
        nFiles = self.data_df.shape[0]
        for f in tqdm(range(nFiles)):
            if self.data_df['with_imaging'][f]:
                analysis = self.data_df['fluo_analysis'][f]
                ...
                if ifPSTH:
                    ...
                    n_colors = 8
                    colors = [(5/255, 105/255, 67/255),
                              (9/255, 153/255, 99/255),
                              (118/255, 183/255, 125/255),
                              (173/255, 212/255, 160/255),
                              (231/255, 168/255, 251/255),
                              (192/255, 109/255, 220/255),
                              (158/255, 5/255, 209/255),
                              (107/255, 3/255, 141/255)]
                    ...
                    colors_combined = {}
                    colors_combined['go'] = colors[1]
                    colors_combined['nogo'] = colors[-2]
            ...
            # calculate selectivity index
                ...
                for ii in range(nCells):
                    sel_index[ii,:]= (PSTH_combined['go',ii]['bootAve'] - PSTH_combined['nogo',ii]['bootAve'])/(PSTH_combined['go',ii]['bootAve'] + PSTH_combined['nogo',ii]['bootAve'])
                ...
                x_vector = np.linspace(-1.95, 1.95, 39)

                # Define the tick positions and labels
                ticks = [-2, -1, 0, 1, 2]  # Desired tick labels
                tick_positions = np.interp(ticks, x_vector, np.arange(len(x_vector)))  # Map ticks to column indices

                # custom colors
                colors_heatmap = [
                    (158/255, 5/255, 209/255),  # Purple for -1
                    (1, 1, 1),  # White for 0
                    (9 / 255, 153 / 255, 99 / 255)  # Green for 1
                ]

                # Create a custom colormap
                custom_cmap = LinearSegmentedColormap.from_list("custom_cmap", colors_heatmap, N=256)

                plt.figure(figsize=(8,10))
                sns.heatmap(sorted_selectivity,vmin=-1, vmax=1, cmap=custom_cmap, cbar=True)
                plt.axvline(x=np.where(x_vector >= 0)[0][0], color='black', linestyle='-', linewidth=2)
                ...
                # plot selectivity for cue/choice/run neurons only
                MLRFile = os.path.join(saveDataPath, 'MLRResult_noR.pickle')
                with open(MLRFile, 'rb') as pf:
                    MLRResult = pickle.load(pf)
                plotVar = ['choice', 'running', 'stimulus']
                for pV in plotVar:
                    selectivity = sel_index[MLRResult['sigCells'][pV]]
                    center=centers_of_mass[MLRResult['sigCells'][pV]]
                    sorted_indices = np.argsort(center)
                    sorted_selectivity = selectivity[sorted_indices]
                    ...
                    plt.xticks(tick_positions, ticks)
                    plt.title('Sorted selectivity index for ' + pV)
                    plt.xlabel('Time from cue')
                    plt.ylabel('Cells sorted')

                    plt.savefig(os.path.join(savePSTHFolder, 'Selectivity_'+pV+'.png'), format='png',
                                dpi=300)
                    plt.savefig(os.path.join(savePSTHFolder, 'Selectivity_'+pV+'.svg'), format='svg')
                    plt.close()
    ...
    def MLR_summary(self, group_adt, group_juv, group_label):
        ...
        sigCells_ADT = np.zeros((3, nFiles)) # number of signifcant cells for stimulus and choice
        ...
        for f in tqdm(range(nFiles)):
            ...
            if f==0:
                ...
                sigCells_ADT[0,f] = len(MLRResult['sigCells']['stimulus'])/MLRResult['coeff'].shape[2]
                sigCells_ADT[1,f] = len(MLRResult['sigCells']['choice'])/MLRResult['coeff'].shape[2]
                sigCells_ADT[2,f] = len(MLRResult['sigCells']['running'])/MLRResult['coeff'].shape[2]
        ...
        sigCells_JUV = np.zeros((3, nFiles))
        # read coefficient, sig cells, neurons with
        for f in tqdm(range(nFiles)):
            ...
            if f==0:
                ...
                sigCells_JUV[0, f] = len(MLRResult['sigCells']['stimulus'])/MLRResult['coeff'].shape[2]
                sigCells_JUV[1, f] = len(MLRResult['sigCells']['choice'])/MLRResult['coeff'].shape[2]
                sigCells_JUV[2,f] = len(MLRResult['sigCells']['running'])/MLRResult['coeff'].shape[2]
            pThresh = 0.01
            nCell = MLRResult['coeff'].shape[2]
            fracSig = np.sum(MLRResult['pval'][:, :, :] < pThresh, 2) / nCell
 ...
 #                                                      [0.5, 0.5], color=(255 / 255, 189 / 255, 53 / 255), linewidth=5)
        ...
        # count number of neurons that significantly encode stimulus and choice
    ...
    def decoding_summary_earlySessionsCtrl(self, group_adt, group_juv, classifier, spec):
        nIdx = 7 # get 80 neurons
        # do 60 neurons later

        ADT_sessions = ['ADT008_220422', 'ADT009_220415', 'ADT010_220413', 'ADT010_220414', 'ADT010_220415', 'ADT028_231209', 'ADT028_231210', 'ADT028_231211', 'ADT029_231215']
        JUV_sessions = ['JUV014_220405', 'JUV014_220406', 'JUV014_220407', 'JUV015_220408', 'JUV015_220409', 'JUV015_220410', 'JUV016_220407', 'JUV016_220408', 'JUV016_220409',
                        'JUV017_221031', 'JUV017_221101', 'JUV017_221102', 'JUV022_230201', 'JUV025_230301', 'JUV025_230302', 'JUV025_230303']
        ...
        for f in tqdm(range(nFiles)):
            ...
            for var in decodingVars:
                sessionID = group_adt.iloc[f]['subject']+'_' + group_adt.iloc[f]['date']
                if sessionID in ADT_sessions:
            # calculate fraction of neurons that are significant
                    decodingSummary_ADT[var]['accuracy'] = np.concatenate((decodingSummary_ADT[var]['accuracy'],
                                                            decodingResult[var]['accuracy'][:, np.newaxis]), 1)
                    decodingSummary_ADT[var]['ctrl_accuracy'] = np.concatenate((decodingSummary_ADT[var]['ctrl_accuracy'],
        ...
        for f in tqdm(range(nFiles)):
            ...
            for var in decodingVars:
                sessionID = group_juv.iloc[f]['subject'] + '_' + group_juv.iloc[f]['date']
                if sessionID in JUV_sessions:
            # calculate fraction of neurons that are significant
                    decodingSummary_JUV[var]['accuracy'] = np.concatenate((decodingSummary_JUV[var]['accuracy'],
                                                            decodingResult[var]['accuracy'][:, np.newaxis]), 1)
                    decodingSummary_JUV[var]['ctrl_accuracy'] = np.concatenate((decodingSummary_JUV[var]['ctrl_accuracy'],
        ...
        savefigpath = os.path.join(self.root_dir, self.summary_dir, 'fluo')
        self.plot_decoding_summary(decodingSummary_ADT, decodingSummary_JUV, regr_time, savefigpath, classifier, nIdx,label='earlyCtrl')
    ...
    def decoding_summary_running(self, group_adt, group_juv,group_label):
        ...
        savefigpath = os.path.join(self.root_dir, self.summary_dir, 'fluo')
        self.plot_decoding_summary(decodingSummary_ADT, decodingSummary_JUV, regr_time, savefigpath, group_label, label = 'earlyCtrl')

    def plot_decoding_summary(self, decoding_ADT, decoding_JUV, regr_time, saveFigPath, classifier, nIdx, label=None):
        #matplotlib.use('Qt5Agg')
        ...
        # Identify significant results
                decodingPlot.save_plot('Decoding accuracy for neurons ' + str((nIdx+1)*10) + var+'_'+classifier+label+'.png',
                                       'png', saveFigPath)
                decodingPlot.save_plot('Decoding accuracy for neurons ' + str((nIdx+1)*10) + var+'_'+classifier+label+'.svg',
                                       'svg', saveFigPath)
        ...
        # save the average decoding accuracy for every session
                ...
                with open(os.path.join(saveFigPath, 'decoding_accuracy_1sWindow_allSession_'+var+'_'+classifier+label+'.pickle'), 'wb') as f:
                    # save as pickle file
                    pickle.dump(data,f)
                f.close()
       ...
       # stats
        ...
        # plot data and ctrl
            ...
            print(anova_table)
            anova_table.to_csv(os.path.join(saveFigPath, 'Ensemble with ' + str((nIdx+1)*10) + ' ANOVA_' + key + label+'.csv'))
        ...
        print(anova_table)
        anova_table.to_csv(
            os.path.join(saveFigPath, 'Ensemble with ' + str((nIdx + 1) * 10) + label+'outcome_ANOVA.csv'))

        plt.savefig(os.path.join(saveFigPath, 'Average decoding accuracy trialType boxplot'+label+'.png'),format='png')
        plt.savefig(os.path.join(saveFigPath, 'Average decoding accuracy trialType boxplot'+label+'.svg'), format='svg')
    ...
    def pseudo_summary(self, group_adt, group_juv,group_label):
        ...
        # plot pseuso decoding accuracy for adult and adol. together
        # and run ANOVA to determine the signficance
        decodingPlot = StartPlots()
        decodingPlot.ax.plot(result_ADT['time'], bootADT_pseudo['bootAve'], color=realColor, label = 'adult')
        decodingPlot.ax.plot(result_ADT['time'], bootJUV_pseudo['bootAve'], color = pseudoColor, label = 'adol.')
        decodingPlot.ax.fill_between(result_ADT['time'], bootADT_pseudo['bootLow'],
                                                        bootADT_pseudo['bootHigh'],
                                                        alpha=0.2, color=realColor)
        decodingPlot.ax.fill_between(result_ADT['time'], bootJUV_pseudo['bootLow'],
                                                bootJUV_pseudo['bootHigh'],
                                                alpha=0.2, color=pseudoColor)
        decodingPlot.ax.set_ylim([0.3,1.05])
        decodingPlot.ax.set_ylabel('Decoding accuracy')
        decodingPlot.ax.set_xlabel('Time from cue(s)')
        decodingPlot.ax.set_title('Shuffled')
        decodingPlot.ax.legend()

        decodingPlot.save_plot('Average decoding accuracy pseudo n=80.png', 'png',
                               savefigpath)
        decodingPlot.save_plot('Average decoding accuracy pseudo n=80.svg', 'svg',
                               savefigpath)

        # anova
        resp_ADT = result_ADT['accuracy_pseudo'].T.flatten()  # (subjects × time) flattened
        resp_JUV = result_JUV['accuracy_pseudo'].T.flatten()

        resp_anova = np.concatenate([resp_ADT, resp_JUV])

        # Age labels
        age_ADT = ['ADT'] * resp_ADT.shape[0]
        age_JUV = ['JUV'] * resp_JUV.shape[0]
        age_anova = age_ADT + age_JUV

        # Time labels (repeat for each subject/sample)
        n_sub_ADT = result_ADT['accuracy_pseudo'].shape[1]
        n_sub_JUV = result_JUV['accuracy_pseudo'].shape[1]
        time_anova = list(result_ADT['time']) * n_sub_ADT + list(result_JUV['time']) * n_sub_JUV

        nSub_ADT = result_ADT['accuracy_pseudo'].shape[1]  # columns = subjects
        nSub_JUV = result_JUV['accuracy_pseudo'].shape[1]

        nTime = result_ADT['accuracy_pseudo'].shape[0]  # rows = time bins

        subject_anova = []

        # Adults
        for s in range(nSub_ADT):
            subject_anova += [f"ADT_{s + 1}"] * nTime

        # Juveniles
        for s in range(nSub_JUV):
            subject_anova += [f"JUV_{s + 1}"] * nTime

        # Build DataFrame
        anova_data = pd.DataFrame({
            "response": resp_anova,  # decoding accuracy (flattened)
            "age": age_anova,  # categorical: ADT / JUV
            "time": time_anova,  # continuous (or categorical if discrete bins)
            "subject": subject_anova  # subject ID (random effect)
        })

        # Fit linear mixed model
        # Random intercept per subject, fixed effects: age, time, and interaction
        model = smf.mixedlm("response ~ C(age)*time", data=anova_data, groups=anova_data["subject"])
        lmm_result = model.fit(reml=False)
...
if __name__ == "__main__":
    ...
    if test_summary:
        ...
        #fluo_summary.selectivity_session(ifPSTH=True)
        ...
        #fluo_summary.decoding_summary_earlySessionsCtrl(ADT_late, JUV_late, classifier, spec)
